...
class AIOKafkaClient:
    ...
    def __init__(self, *, loop, bootstrap_servers='localhost',
        ...
        if security_protocol not in ('SSL', 'PLAINTEXT',
            ...
            raise ValueError("`security_protocol` should be "
        ...
        if security_protocol in ('SASL_PLAINTEXT', 'SASL_SSL'):
            assert sasl_mechanism in ('PLAIN', 'GSSAPI'), \
                'sasl_mechanism must be PLAIN or GSSAPI '
            if sasl_mechanism == 'PLAIN':
                assert sasl_plain_username is not None, \
                    'sasl_plain_username required for PLAIN sasl'
                assert sasl_plain_password is not None, \
        ...
        try:
            ...
            if self._sasl_mechanism not in response.enabled_mechanisms:
                ...
                if self._security_protocol == 'SASL_PLAINTEXT':
                    log.warning(
                        '%s: Sending username and password in the clear', self)
                ...
                try:
                    ...
                    raise Errors.ConnectionError(
                        "Connection at {0}:{1} broken: {2}".format(
                            conn._host, conn._port, err))

                try:
                    # The server will send a zero sized message  (that is
                    # Int32(0)) on success. The connection is closed on failure
                    correlation_id = conn._next_correlation_id()
                    fut = create_future(loop=self._loop)
                    conn._requests.append((correlation_id, None, fut))
                    yield from asyncio.wait_for(
                        fut, self._request_timeout_ms, loop=self._loop)
                except ConnectionError as err:
                    log.exception("%s: Error receiving reply from server, %s",
                                  self, err)
                ...
                log.info('%s: Authenticated as %s via PLAIN',
                         self, self._sasl_plain_username)
            elif self._sasl_mechanism == 'GSSAPI':
                raise KafkaError(
                    'GSSAPI sasl mechanism is not yet supported by aiokafka')
            else:
                raise Errors.UnsupportedSaslMechanismError(
            ...
            log.warning('Unable to do sasl authentication from "%s:%s": %s',
                        conn._host, conn._port, err)
            return False
