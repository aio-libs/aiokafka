...
class AIOKafkaConsumer(object):
    ...
    def subscribe(self, topics=(), pattern=None, listener=None):
        ...
        if pattern is not None:
            ...
            if self._group_id is None:
                # We have reset the assignment, but client.set_topics will not always
                # do a metadata update. We force it to do it even if metadata did not
                # change. This will trigger a reassignment on NoGroupCoordinator, but
                # only if snapshot did not change, thus we reset it too.
