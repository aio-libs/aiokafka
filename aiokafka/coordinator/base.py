...
class BaseCoordinator(object):
    ...
    def __init__(self, client, *, loop, **configs):
        """
        Keyword Arguments:
    ...
    def _send_req(self, node_id, request):
        """send request to Kafka and mark coordinator as `dead` in error case
        """
        try:
        ...
        while (yield from self.coordinator_unknown()):
            ...
            try:
                ...
                if not (yield from self.coordinator_unknown()):
                    ...
                    log.debug("Coordinator already known"
                              " -- ignoring metadata response")
                    break
                self.coordinator_id = resp.coordinator_id
                yield from self._client.ready(self.coordinator_id)
    ...
    def ensure_active_group(self):
        ...
        if self.needs_join_prepare:
            yield from self._on_join_prepare(self.generation, self.member_id)
    ...
    def _heartbeat_task_routine(self):
        ...
        hb_interval = self.config['heartbeat_interval_ms'] / 1000
        session_timeout = self.config['session_timeout_ms'] / 1000
        retry_backoff_time = self.config['retry_backoff_ms'] / 1000
        sleep_time = hb_interval

        while True:
            yield from asyncio.sleep(sleep_time, loop=self._client.loop)
            ...
            try:
                ...
                log.error("Heartbeat failed: authorization error: %s", err)
            except Errors.KafkaError as err:
