...
from aiokafka.coordinator.consumer import ConsumerCoordinator
...
class AIOKafkaConsumer(object):
    ...
    DEFAULT_CONFIG = {
        ...
        'client_id': 'aiokafka-python-' + __version__,
        'group_id': 'aiokafka-python-default-group',
    ...
    def __init__(self, *topics, loop, **configs):
        ...
        self._client = AIOKafkaClient(loop=loop, **self.config)
    ...
    def start(self):
        ...
        if self.config['api_version'] == 'auto':
            self.config['api_version'] = \
                yield from self._client.check_version()
        ...
        self._fetcher = Fetcher(
            self._client, self._subscription, loop=self._loop, **self.config)
    ...
    def pause(self, *partitions):
        ...
        partitions until they have been resumed using resume().
        Note that this method does not affect partition subscription.
        In particular, it does not cause a group rebalance when
        automatic assignment is used.
    ...
    def get_message(self):
        ...
        while True:
            ...
            msg = yield from self._fetcher.next_record()
            if msg is None:
