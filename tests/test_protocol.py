...
# Expected encodings for the zig-zag varint types, taken from the boundary
# values of every varint width in both signs. They are the same vectors Kafka's
# own ByteUtilsTest uses for writeVarint/writeVarlong.
VARINT32_SERDE_DATA = [
    (0, [0x00]),
    (-1, [0x01]),
    (1, [0x02]),
...
VARINT64_SERDE_DATA = [
    *VARINT32_SERDE_DATA,
    ...
    (-281474976710657, [0x81, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x01]),
...
@pytest.mark.parametrize(("value", "expected_encoded"), VARINT32_SERDE_DATA)
def test_signed_varint_serde(value: int, expected_encoded: list[int]) -> None:
    encoded = VarInt32.encode(value)
    assert encoded == b"".join(struct.pack(">B", x) for x in expected_encoded)
    assert value == VarInt32.decode(io.BytesIO(encoded))


@pytest.mark.parametrize(("value", "expected_encoded"), VARINT64_SERDE_DATA)
def test_signed_varlong_serde(value: int, expected_encoded: list[int]) -> None:
