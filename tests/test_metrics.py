...
from aiokafka.metrics import DictReporter, MetricConfig, MetricName, Metrics, Quota
from aiokafka.metrics.measurable import AbstractMeasurable
from aiokafka.metrics.stats import (
    Avg,
    Count,
    Max,
    Min,
...
from aiokafka.metrics.stats.percentiles import BucketSizing
from aiokafka.metrics.stats.rate import TimeUnit
...
def test_simple_stats(mocker, time_keeper, config, metrics):
    ...
    sensor.add(
        metrics.metric_name("test.occurences", "grp1"), Rate(TimeUnit.SECONDS, Count())
    )
    sensor.add(metrics.metric_name("test.count", "grp1"), Count())
    percentiles = [
        Percentile(metrics.metric_name("test.median", "grp1"), 50.0),
        Percentile(metrics.metric_name("test.perc99_9", "grp1"), 99.9),
    ]
    sensor.add_compound(
        Percentiles(100, BucketSizing.CONSTANT, 100, -100, percentiles=percentiles)
    )
    ...
    assert (
        abs(
            count / elapsed_secs
            - metrics.metrics.get(
                metrics.metric_name("test.occurences", "grp1")
            ).value()
        )
        < EPS
    ), "Occurrences(0...%d) = %f" % (count, count / elapsed_secs)
    ...
    assert (
        abs(5.0 - metrics.metrics.get(metrics.metric_name("s2.total", "grp1")).value())
        < EPS
    ), "s2 reflects the constant value"
    assert (
        abs(4.5 - metrics.metrics.get(metrics.metric_name("test.avg", "grp1")).value())
        < EPS
    ), "Avg(0...9) = 4.5"
    assert (
        abs(
            (count - 1)
            - metrics.metrics.get(metrics.metric_name("test.max", "grp1")).value()
        )
        < EPS
    ), "Max(0...9) = 9"
    assert (
        abs(0.0 - metrics.metrics.get(metrics.metric_name("test.min", "grp1")).value())
        < EPS
    ), "Min(0...9) = 0"
    assert (
        abs(
            (sum_val / elapsed_secs)
            - metrics.metrics.get(metrics.metric_name("test.rate", "grp1")).value()
        )
        < EPS
    ), "Rate(0...9) = 1.40625"
    assert (
    ...
    assert (
        abs(
            count
            - metrics.metrics.get(metrics.metric_name("test.count", "grp1")).value()
        )
        < EPS
    ), "Count(0...9) = 10"
...
def test_remove_sensor(metrics):
    ...
    assert (
        metrics.metrics.get(metrics.metric_name("test.parent1.count", "grp1")) is None
    )
    ...
    assert metrics._children_sensors.get(sensor) is None
    assert (
        metrics.metrics.get(metrics.metric_name("test.gchild2.count", "grp1")) is None
    )
    ...
    assert metrics._children_sensors.get(sensor) is None
    assert (
        metrics.metrics.get(metrics.metric_name("test.parent2.count", "grp1")) is None
    )
...
def test_remove_inactive_metrics(mocker, time_keeper, metrics):
    ...
    purger.run(metrics)
    assert metrics.get_sensor("test.s1") is not None, "Sensor test.s1 must be present"
    assert (
        metrics.metrics.get(metrics.metric_name("test.s1.count", "grp1")) is not None
    ), "MetricName test.s1.count must be present"
    assert metrics.get_sensor("test.s2") is not None, "Sensor test.s2 must be present"
    assert (
        metrics.metrics.get(metrics.metric_name("test.s2.count", "grp1")) is not None
    ), "MetricName test.s2.count must be present"

    time_keeper.sleep(1.001)
    purger.run(metrics)
    assert (
        metrics.get_sensor("test.s1") is None
    ), "Sensor test.s1 should have been purged"
    assert (
        metrics.metrics.get(metrics.metric_name("test.s1.count", "grp1")) is None
...
def test_Quotas(metrics):
    ...
    assert (
        abs(
            6.0
            - metrics.metrics.get(metrics.metric_name("test1.total", "grp1")).value()
        )
        < EPS
    )
...
def test_Percentiles(metrics):
    ...
    percs = Percentiles(
        4 * buckets, BucketSizing.CONSTANT, 100.0, 0.0, percentiles=_percentiles
    )
...
def test_rate_windowing(mocker, time_keeper, metrics):
    ...
    # prior to any time passing
    elapsed_secs = (
        config.time_window_ms * (config.samples - 1) + config.time_window_ms / 2.0
    ) / 1000.0

    kafka_metric = metrics.metrics.get(metrics.metric_name("test.rate", "grp1"))
    assert (
        abs((sum_val / elapsed_secs) - kafka_metric.value()) < EPS
    ), "Rate(0...2) = 2.666"
    assert (
        abs(
            elapsed_secs
            - (kafka_metric.measurable.window_size(config, time.time() * 1000) / 1000.0)
        )
        < EPS
...
def test_reporter(metrics):
    ...
    sensor2.add(
        metrics.metric_name("pack.bean2.some", "grp1", tags={"a": 42, "b": "bar"}),
        Total(),
    )
